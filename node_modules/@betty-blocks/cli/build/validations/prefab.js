...
const schemaProvider = (styles, componentStyleMap, prefabType) => {
    return joi_1.default.object({
        ...
        interactions: joi_1.default.array().items(interaction_1.interactionSchema).max(constants_1.MAX_INTERACTIONS),
        actions: joi_1.default.array().items(action_1.actionSchema).max(constants_1.MAX_ACTIONS),
        variables: joi_1.default.array().items(variable_1.variableSchema).max(constants_1.MAX_VARIABLES),
        beforeCreate: joi_1.default.any(),
        structure: joi_1.default.array()
            .items(joi_1.default.custom(component_1.validateComponent(styles, componentStyleMap, prefabType)))
            .required(),
        reconfigure: joi_1.default.any(),
    });
};
const validate = (styles, componentStyleMap, prefabType) => (prefab) => {
    const { actions, variables } = prefab;
    const { error } = schemaProvider(styles, componentStyleMap, prefabType).validate(prefab);
...
exports.default = (prefabs, styles, componentStyleMap, prefabType) => {
